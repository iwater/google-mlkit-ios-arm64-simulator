...
这些符号在 GoogleMLKit 伞包中由链接器统一解析，但单独使用 xcframework 时需要提供 stub 实现。项目中 `ocrexample/AbseilStubs.mm` 提供了这些符号的 stub 实现。
...
3. **推送 Spec 仓库**：在 GitHub 上建一个轻量的 Specs 仓库（如 `raz-cocoapods-specs`），将本地 `Specs/` 目录下的内容初始化为 Git 仓库并推送到远程。
4. **宿主 App `Podfile` 引入**：
   在宿主项目的 `Podfile` 顶部引入您的私有源，然后像普通 Pod 依赖一样声明即可：
   ```ruby
   source 'https://github.com/iwater/raz-cocoapods-specs.git' # 您的私有 specs 仓库
...
*(注意：即使采用私有 Spec Repo 方式，由于 CocoaPods 静态二进制导入存在头文件路径的缺陷，您依然需要在 `Podfile` 中保留下方“方式二”中的 `post_install` 钩子，以补全 Framework 头文件搜索路径。)*
...
**注意事项：**
- GoogleDataTransport 在 Xcode 15+ 下会因 `-Werror,-Wstrict-prototypes` 编译失败，需在 `post_install` 中添加 `-Wno-strict-prototypes`
...
|   +-- MLKitCommon/
|   |   +-- MLKitCommon.podspec  # 含依赖声明（GoogleUtilities, GTMSessionFetcher 等）
